...
> **Extension:** Falls `.claude/3-project/ha-docker-ext.md` existiert → jetzt sofort lesen und vollständig anwenden.
...
| Dev-Stack | docker-compose.dev.yml | Home Assistant + MQTT + InfluxDB (lokal) |
| Prod-Stack | docker-compose.prod.yml | Proxmox/Unraid native (Docker Add-ons) |
...
Home Assistant wird beim Start automatisch die config/configuration.yaml laden und alle Packages in config/packages/ initialisieren.
